describe('unit test for quz.js', function () {
    ...
    describe('#sub()', function () {
        it('should get the sublist consisting of LEN items from list L', function () {
            C.toString().should.equal('[ 19 8 7 6 7 ]');
            sub(C, 3, 2).toString().should.equal('[ 6 7 ]');
            sub(C, 1, 1).toString().should.equal('[ 8 ]');
            sub(C, 0, 1).toString().should.equal('[ 19 ]');
            sub(C, 0, 5).toString().should.equal('[ 19 8 7 6 7 ]');
            C.toString().should.equal('[ 19 8 7 6 7 ]');
        });
    });
});