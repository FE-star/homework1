describe('unit test for quz.js', function () {
    ...
    describe('#sub()', function () {
        it('should get the sublist consisting of LEN items from list L', function () {
            ...
            sub(C, 0, 5).toString().should.equal('[ 19 8 7 3 2 ]');
            sub(C, -1, 5).toString().should.equal('[ 19 8 7 3 2 ]');
            sub(C, -1, 10).toString().should.equal('[ 19 8 7 3 2 ]');
            sub(C, -1, -1).toString().should.equal('[ 0 ]');
            sub(C, -1, 0).toString().should.equal('[ 0 ]');
            sub(C, 10, -1).toString().should.equal('[ 0 ]');
            sub(C, -10, 10).toString().should.equal('[ 19 8 7 3 2 ]');
            C.toString().should.equal('[ 19 8 7 3 2 ]');
        });
    });
